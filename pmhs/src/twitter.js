  ...
  twitterjs.domReady((function(options, guid) {
    return function () {
      function render(tweet) {
        ...
        if (options.prefix) {
          html += '<tr><span className="twitterPrefix">';
          html += options.prefix.replace(/%(.*?)%/g, function (m, l) {
              return tweet.user[l];
          });
          html += ' </span></tr>'; // space on purpose
        }
      ...
      function getTweets() {
        ...
        twitterlib[searchMethod](id, tlOptions, function (tweets, tlOptions) {
          ...
          for (var i = 0; i < max; i++) {
            ...
            if (options.template) {
              html.push('<tr>' + options.template.replace(/%([a-z_\-\.]*)%/ig, function (m, l) {
                var r = tweets[i][l] + "" || "";
      ...
      if (typeof twitterlib !== 'function') {
        ...
        setTimeout(function () {
          ...
          script.src = 'src/twitterlib.min.js';
