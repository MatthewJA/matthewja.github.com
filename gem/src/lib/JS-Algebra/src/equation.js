...
(function() {
  ...
  define(["JSAlgebra/variable", "JSAlgebra/constant", "JSAlgebra/algebraException"], function(Variable, Constant, AlgebraException) {
    ...
    return Equation = (function() {
      ...
      Equation.prototype.collectTerms = function(equivalencies) {
        var equivalency, equivalentTerm, leftConstant, leftTerms, leftTermsList, refEquation, rightConstant, rightTerms, rightTermsList, root, term, termEquivalencies, _i, _j, _k, _l, _len, _len1, _len2, _len3, _ref, _ref1;
        ...
        refEquation = this.collectConstants();
        leftTerms = {};
        leftConstant = null;
        _ref = refEquation.leftTerms;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          term = _ref[_i];
          if (term.isVariable != null) {
            termEquivalencies = equivalencies.get(term.label);
            for (_j = 0, _len1 = termEquivalencies.length; _j < _len1; _j++) {
              equivalency = termEquivalencies[_j];
              if (equivalency in leftTerms) {
        ...
        rightTerms = {};
        rightConstant = null;
        _ref1 = refEquation.rightTerms;
        for (_k = 0, _len2 = _ref1.length; _k < _len2; _k++) {
          term = _ref1[_k];
          if (term.isVariable != null) {
            termEquivalencies = equivalencies.get(term.label);
            for (_l = 0, _len3 = termEquivalencies.length; _l < _len3; _l++) {
              equivalency = termEquivalencies[_l];
              if (equivalency in rightTerms) {
